...
class Application {
	...
	*/
	Application(int window_width, int window_height, std::string window_title)
		...
		if ((scale = SDL_GetDisplayContentScale(SDL_GetPrimaryDisplay())) ==
			0) {
		...
		window =
			SDL_CreateWindow(window_title.c_str(), (int)(window_width * scale),
	...
	*/
	...
	~Application() {
		...
		initialising that much stuff so it probably does not matter.
		*/
		if (renderer != nullptr) {
...
int main() {
	...
	if (status == true) {
		...
		// Checking the return value is unnecessary since main() exits
