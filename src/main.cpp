...
enum ApplicationStatus {
  SUCCESS,              // Success.
  INITIALIZATION_ERROR, // An error occurred during setup, e.g. when creating a
                        // window. (SDL errors.)
  RUNTIME_ERROR // An error occurred during runtime, e.g. when loading a file.
};

// File dialog filters (standard template from SDL-wiki, replace with desired
// formats)
...
public:
  /*
  Print a human-readable message reflecting the current status of the
  application.
  ...
  */
  ...
  returned object before using it to find out if initialisation has failed!
  */
  Application(int window_width, int window_height, std::string window_title)
    ...
    if ((scale = SDL_GetDisplayContentScale(SDL_GetPrimaryDisplay())) == 0) {
      scale = 1; // Use the scaling factor expected by the display based on its
                 // DPI settings, default to 1.
    }
    SDL_WindowFlags flags =
        SDL_WINDOW_HIDDEN | SDL_WINDOW_HIGH_PIXEL_DENSITY |
        SDL_WINDOW_RESIZABLE; // The window must be shown explicitly.
    window = SDL_CreateWindow(window_title.c_str(), (int)(window_width * scale),
                              (int)(window_height * scale), flags);
    if (window == nullptr) {
      status = INITIALIZATION_ERROR;
      return;
    }
  ...
  @return The `status` value of the `Application` object is set by the function
  to reflect the state of the application when the main loop has been
  terminated.
  */
  void run() {
    ...
    while (quit == false) {
      ...
      if (ImGui::Button("Button A")) {
        ...
        SDL_ShowOpenFileDialog(callback, nullptr, window, filters,
                               SDL_arraysize(filters), nullptr, true);
      }
      ImGui::End();
...
int main() {
  ...
  if (status == true) {
    ...
    // Checking the return value is unnecessary since main() exits immediately
    // afterwards.
